...
{
    /* Preserve handlers for TLS PRF p_hash to avoid re-allocation */
...
{
    /* Preserve handlers for handshake hash states to avoid re-allocation */
    ...
    hash_handles->sslv3_md5_copy = conn->handshake.sslv3_md5_copy.digest.evp;
    hash_handles->sslv3_md5_copy_hash_impl = conn->handshake.sslv3_md5_copy.hash_impl;
    hash_handles->sslv3_sha1_copy = conn->handshake.sslv3_sha1_copy.digest.evp;
    hash_handles->sslv3_sha1_copy_hash_impl = conn->handshake.sslv3_sha1_copy.hash_impl;
    hash_handles->tls_hash_copy = conn->handshake.tls_hash_copy.digest.evp;
    hash_handles->tls_hash_copy_hash_impl = conn->handshake.tls_hash_copy.hash_impl;

    /* Preserve handlers for SSLv3 PRF hash states to avoid re-allocation */
    ...
    /* Preserve handlers for initial signature hash states to avoid re-allocation */
    ...
    /* Preserve handlers for secure signature hash states to avoid re-allocation */
...
{
    /* Preserve handlers for initial client mac hmac states to avoid re-allocation */
    ...
    /* Preserve handlers for initial client mac copy hmac states to avoid re-allocation */
    ...
    /* Preserve handlers for initial server mac hmac states to avoid re-allocation */
    ...
    /* Preserve handlers for secure client mac hmac states to avoid re-allocation */
    ...
    /* Preserve handlers for secure client mac copy hmac states to avoid re-allocation */
    ...
    /* Preserve handlers for secure server mac hmac states to avoid re-allocation */
...
int s2n_connection_restore_hash_state(struct s2n_connection *conn, struct s2n_connection_hash_handles *hash_handles)
{
    ...
    conn->handshake.sslv3_md5_copy.digest.evp = hash_handles->sslv3_md5_copy;
    conn->handshake.sslv3_md5_copy.hash_impl = hash_handles->sslv3_md5_copy_hash_impl;
    conn->handshake.sslv3_sha1_copy.digest.evp = hash_handles->sslv3_sha1_copy;
    conn->handshake.sslv3_sha1_copy.hash_impl = hash_handles->sslv3_sha1_copy_hash_impl;
    conn->handshake.tls_hash_copy.digest.evp = hash_handles->tls_hash_copy;
...
int s2n_connection_restore_hmac_state(struct s2n_connection *conn, struct s2n_connection_hmac_handles *hmac_handles)
{
    ...
    /* Restore s2n_connection handlers for initial client record mac copy */
    ...
    /* Restore s2n_connection handlers for secure client record mac copy */
