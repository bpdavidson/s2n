...
static int s2n_evp_p_hash_new(struct s2n_prf_working_space *ws)
{
    notnull_check(ws->tls.p_hash.evp_hmac.evp_digest.ctx = S2N_EVP_MD_CTX_NEW());
    return 0;
}

static int s2n_evp_p_hash_digest_init(struct s2n_prf_working_space *ws)
{
...
static int s2n_evp_p_hash_init(struct s2n_prf_working_space *ws, s2n_hmac_algorithm alg, struct s2n_blob *secret)
{
    ...
    return s2n_evp_p_hash_digest_init(ws);
}

static int s2n_evp_p_hash_update(struct s2n_prf_working_space *ws, const void *data, uint32_t size)
{
...
static int s2n_evp_p_hash_digest(struct s2n_prf_working_space *ws, void *digest, uint32_t size)
{
...
static int s2n_evp_p_hash_wipe(struct s2n_prf_working_space *ws)
{
...
static int s2n_evp_p_hash_reset(struct s2n_prf_working_space *ws)
{
    GUARD(s2n_evp_p_hash_wipe(ws));

    return s2n_evp_p_hash_digest_init(ws);
}

static int s2n_evp_p_hash_cleanup(struct s2n_prf_working_space *ws)
{
...
static int s2n_evp_p_hash_free(struct s2n_prf_working_space *ws)
{
    notnull_check(ws->tls.p_hash.evp_hmac.evp_digest.ctx);
...
const struct s2n_p_hash_implementation s2n_evp_p_hash = {
    .new = &s2n_evp_p_hash_new,
    .init = &s2n_evp_p_hash_init,
    .update = &s2n_evp_p_hash_update,
    .final = &s2n_evp_p_hash_digest,
    .reset = &s2n_evp_p_hash_reset,
    .cleanup = &s2n_evp_p_hash_cleanup,
    .free = &s2n_evp_p_hash_free,
};

const struct s2n_p_hash_implementation s2n_hmac_p_hash = {
                      ...
                      struct s2n_blob *label, struct s2n_blob *seed_a, struct s2n_blob *seed_b, struct s2n_blob *out)
{
    ...
    const struct s2n_p_hash_implementation *signed_digest = ws->tls.p_hash_impl;
...
int s2n_prf_new(struct s2n_connection *conn) {
    if (s2n_is_in_fips_mode()) {
        ...
        conn->prf_space.tls.p_hash_impl = &s2n_evp_p_hash;
    } else {
        /* Aside from FIPS mode, the formally verified s2n_hmac implementation is used for the PRF */
        conn->prf_space.tls.p_hash_impl = &s2n_hmac_p_hash;
    }

    return conn->prf_space.tls.p_hash_impl->new(&conn->prf_space);
}

int s2n_prf_free(struct s2n_connection *conn) {
    return conn->prf_space.tls.p_hash_impl->free(&conn->prf_space);
}
...
static int s2n_sslv3_client_finished(struct s2n_connection *conn)
{
    ...
    lte_check(MD5_DIGEST_LENGTH + SHA_DIGEST_LENGTH, sizeof(conn->handshake.client_finished));
    GUARD(s2n_hash_copy(&conn->handshake.sslv3_md5_copy, &conn->handshake.md5));
    GUARD(s2n_hash_copy(&conn->handshake.sslv3_sha1_copy, &conn->handshake.sha1));
    return s2n_sslv3_finished(conn, prefix, &conn->handshake.sslv3_md5_copy, &conn->handshake.sslv3_sha1_copy, conn->handshake.client_finished);
}
...
static int s2n_sslv3_server_finished(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_hash_copy(&conn->handshake.sslv3_md5_copy, &conn->handshake.md5));
    GUARD(s2n_hash_copy(&conn->handshake.sslv3_sha1_copy, &conn->handshake.sha1));
    return s2n_sslv3_finished(conn, prefix, &conn->handshake.sslv3_md5_copy, &conn->handshake.sslv3_sha1_copy, conn->handshake.server_finished);
}
...
int s2n_prf_client_finished(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_hash_digest(&conn->handshake.sslv3_md5_copy, md5_digest, MD5_DIGEST_LENGTH));
    GUARD(s2n_hash_digest(&conn->handshake.sslv3_sha1_copy, sha_digest, SHA_DIGEST_LENGTH));
    md5.data = md5_digest;
