...
static int s2n_evp_hash_new(struct s2n_hash_state *state)
{
    ...
    notnull_check(state->digest.evp.md5_sha1_secondary.ctx = S2N_EVP_MD_CTX_NEW());
...
static int s2n_evp_hash_init(struct s2n_hash_state *state, s2n_hash_algorithm alg)
{
    ...
    switch (alg) {
    case S2N_HASH_NONE:
        state->digest.evp.primary.md = EVP_md_null();
        state->digest.evp.md5_sha1_secondary.md = EVP_md_null();
        break;
    case S2N_HASH_MD5:
        state->digest.evp.primary.md = EVP_md5();
        state->digest.evp.md5_sha1_secondary.md = EVP_md_null();
        break;
    case S2N_HASH_SHA1:
        state->digest.evp.primary.md = EVP_sha1();
        state->digest.evp.md5_sha1_secondary.md = EVP_md_null();
        break;
    case S2N_HASH_SHA224:
        state->digest.evp.primary.md = EVP_sha224();
        state->digest.evp.md5_sha1_secondary.md = EVP_md_null();
        break;
    case S2N_HASH_SHA256:
        state->digest.evp.primary.md = EVP_sha256();
        state->digest.evp.md5_sha1_secondary.md = EVP_md_null();
        break;
    case S2N_HASH_SHA384:
        state->digest.evp.primary.md = EVP_sha384();
        state->digest.evp.md5_sha1_secondary.md = EVP_md_null();
        break;
    case S2N_HASH_SHA512:
        state->digest.evp.primary.md = EVP_sha512();
        state->digest.evp.md5_sha1_secondary.md = EVP_md_null();
        break;
    case S2N_HASH_MD5_SHA1:
        state->digest.evp.primary.md = EVP_sha1();
        state->digest.evp.md5_sha1_secondary.md = EVP_md5();
        break;
    default:
        S2N_ERROR(S2N_ERR_HASH_INVALID_ALGORITHM);
    }

    r = EVP_DigestInit_ex(state->digest.evp.primary.ctx, state->digest.evp.primary.md, NULL);
    r &= EVP_DigestInit_ex(state->digest.evp.md5_sha1_secondary.ctx, state->digest.evp.md5_sha1_secondary.md, NULL);
...
static int s2n_evp_hash_update(struct s2n_hash_state *state, const void *data, uint32_t size)
{
    ...
    case S2N_HASH_MD5_SHA1:
        ...
        r &= EVP_DigestUpdate(state->digest.evp.md5_sha1_secondary.ctx, data, size);
        break;
    default:
        S2N_ERROR(S2N_ERR_HASH_INVALID_ALGORITHM);
    }
...
static int s2n_evp_hash_digest(struct s2n_hash_state *state, void *out, uint32_t size)
{
    ...
    case S2N_HASH_MD5_SHA1:
        ...
        r &= EVP_DigestFinal_ex(state->digest.evp.md5_sha1_secondary.ctx, out, &digest_size);
        break;
    default:
        S2N_ERROR(S2N_ERR_HASH_INVALID_ALGORITHM);
    }
...
static int s2n_evp_hash_copy(struct s2n_hash_state *to, struct s2n_hash_state *from)
{
    ...
    case S2N_HASH_MD5_SHA1:
        ...
        r &= EVP_MD_CTX_copy_ex(to->digest.evp.md5_sha1_secondary.ctx, from->digest.evp.md5_sha1_secondary.ctx);
        break;
    default:
        S2N_ERROR(S2N_ERR_HASH_INVALID_ALGORITHM);
    }
...
static int s2n_evp_hash_reset(struct s2n_hash_state *state)
{
    ...
    r &= S2N_EVP_MD_CTX_RESET(state->digest.evp.md5_sha1_secondary.ctx);
...
static int s2n_evp_hash_free(struct s2n_hash_state *state)
{
    ...
    S2N_EVP_MD_CTX_FREE(state->digest.evp.md5_sha1_secondary.ctx);
    state->digest.evp.primary.ctx = NULL;
    state->digest.evp.md5_sha1_secondary.ctx = NULL;

    return 0;
}

const struct s2n_hash_implementation s2n_low_level_hash = {
...
const struct s2n_hash_implementation s2n_evp_hash = {
