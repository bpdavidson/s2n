...
cd $BUILD_DIR
curl -L http://www.openssl.org/source/openssl-fips-2.0.13.tar.gz > openssl-fips-2.0.13.tar.gz
...
$CONFIGURE -g3 -fPIC no-libunbound no-gmp no-jpake no-krb5 no-md2 no-rc5 no-rfc3779 no-sctp no-ssl-trace \
         ...
         no-dsa no-ssl2 no-capieng -DSSL_FORBID_ENULLi --with-fipsdir=$FIPS_DIR -DOPENSSL_NO_DTLS1 \
